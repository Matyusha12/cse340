...
const vmModel = require('../models/vehicleManagement-model');
...
async function addClassification(req, res) {
    ...
    const regResult = await vmModel.addClassification(classification_name);

    if (regResult) {
        req.flash('notice', `Congratulations, you're registered ${classification_name}. See navigation.`);
...
async function addInventory(req, res) {
    ...
    if (regResult) {
        req.flash('notice', `Congratulations, you're registered ${inv_make} ${inv_model}. See navigation.`);
        res.status(201).render('inventory/management', {
            title: 'Vehicle Management',
            nav,
            errors: null,
...
async function addTestDriveDetails(req, res) {
    ...
    const regResult = await vmModel.addTestDriveDetails(
        tdc_date, tdc_time, tdc_fullname, tdc_cellphone, tdc_make_model
    );

    if (regResult) {
        req.flash('notice', `Congratulations, you have registered ${tdc_make_model} for a test drive. See you there!.`);
